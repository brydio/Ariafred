...
def get_rpc():
    ...
    try:
        ...
        wf.add_item(u'Aria2 may not be running, try starting it?',
                    u'Press Enter',
                    arg=u'--run-aria2',
                    valid=True)
        wf.add_item(u'Or change RPC path?',
                    u'Currently using ' + rpc_path,
                    arg=u'--go-rpc-setting',
                    valid=True)
        wf.add_item(u'Or change RPC secret?',
                    u'Currently using \'' + current_secret + '\'',
                    arg=u'--go-secret-setting',
                    valid=True)
        return False
    else:
        return True
...
def size_fmt(num, suffix='B'):
    for unit in ['', 'Ki', 'Mi', 'Gi', 'Ti']:
        if abs(num) < 1024.0:
            return '{num:.2f} {unit}{suffix}'.format(num=num,
                                                     unit=unit,
                                                     suffix=suffix)
        num /= 1024.0
...
def apply_filter(tasks, filters):
    if filters:
        for filter in filters:
            tasks = wf.filter(filter,
                              tasks,
                              lambda task: get_task_name(task),
                              min_score=20)
    return tasks
...
def get_task_name(task):
    ...
    if bt:
        ...
        if 'bittorrent' in bt and 'info' in bt['bittorrent']:
            bt_name = bt['bittorrent']['info']['name']
        else:
            bt_name = path  # os.path.basename(os.path.dirname(path))
        if not bt_name:
            bt_name = 'Task name not obtained yet'
        name = u'{bt_name} (BT: {file_num} files)'.format(bt_name=bt_name,
                                                          file_num=file_num)
    else:
...
def get_active_tasks(command, filters):
    active = server.tellActive(secret, [
        'gid', 'completedLength', 'totalLength', 'downloadSpeed',
        'uploadSpeed', 'connections'
    ])
...
def get_pending_tasks(command, filters):
    waiting = server.tellWaiting(
        secret, -1, 10, ['gid', 'status', 'completedLength', 'totalLength'])
...
def get_stopped_tasks():
    stopped = server.tellStopped(
        secret, -1, 20,
        ['gid', 'status', 'completedLength', 'totalLength', 'errorMessage'])
    return stopped
...
def get_completed_tasks(command, filters):
    ...
    for task in completed:
        ...
        arg = '--' + command + ' ' + task['gid']
        filepath = server.getFiles(secret,
                                   task['gid'])[0]['path'].encode('utf-8')
        if not os.path.exists(filepath):
            info = '[deleted] ' + info
            item = wf.add_item(name,
                               info,
                               arg=arg,
                               valid=True,
                               icon=icon_deleted)
...
def get_removed_tasks(command, filters):
    ...
    for task in removed:
        ...
        arg = '--' + command + ' ' + task['gid']
        item = wf.add_item(name,
                           u'This task is removed by user.',
                           arg=arg,
                           valid=True,
                           icon=icon_removed)
...
def get_stats():
    if get_rpc():
        ...
        wf.add_item('Stopped: ' + stats['numStopped'],
                    ...
                    valid=True,
                    icon=icon_stopped)
        wf.add_item('Download: ' +
                    size_fmt(int(stats['downloadSpeed']), suffix='B/s'),
                    'Current download limit: {limit} KiB/s'.format(
                        limit=options['max-overall-download-limit']),
                    arg='--go-download-limit-setting',
                    valid=True,
                    icon=icon_download)
        wf.add_item('Upload: ' +
                    size_fmt(int(stats['uploadSpeed']), suffix='B/s'),
                    'Current upload limit: {limit} KiB/s'.format(
                        limit=options['max-overall-upload-limit']),
                    arg='--go-upload-limit-setting',
                    valid=True,
                    icon=icon_upload)
...
def limit_speed(type, param):
    if get_rpc():
        limit = int(
            server.getGlobalOption(secret)['max-overall-' + type + '-limit'])
        limit = speed_convert(limit)[1]
        param_s = speed_convert(param)[1]
        wf.add_item('Limit ' + type +
                    ' speed to: {limit}'.format(limit=param_s),
                    'Current ' + type + ' limit (0 for no limit): ' + limit,
                    arg='--limit-' + type + ' ' + param,
                    valid=True)
...
def limit_num(param):
    if get_rpc():
        ...
        wf.add_item(
            'Limit concurrent downloads to: {limit}'.format(limit=param),
